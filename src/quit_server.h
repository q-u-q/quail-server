#ifndef SRC_QUIT_SERVER_H_
#define SRC_QUIT_SERVER_H_
...
class QuitServer : public QuicSpdyServerBase, public QuicSocketEventListener {
 public:
  // `quic_simple_server_backend` must outlive the created QuicServer.
  QuitServer(std::unique_ptr<ProofSource> proof_source,
  ...
  QuitServer(const QuitServer&) = delete;
  QuitServer& operator=(const QuitServer&) = delete;
