#ifndef SRC_QUAIL_SERVER_BACKEND_H_
#define SRC_QUAIL_SERVER_BACKEND_H_
...
class QuailServerBackend : public quic::QuicSimpleServerBackend {
 public:
  class ResourceFile {
   public:
 ...
 private:
  ...
  std::unique_ptr<quic::QuicBackendResponse> default_response_
      QUIC_GUARDED_BY(response_mutex_);

  // The generate bytes response, if set.
  std::unique_ptr<quic::QuicBackendResponse> generate_bytes_response_
      QUIC_GUARDED_BY(response_mutex_);
