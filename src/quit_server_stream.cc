...
QuitServerStream::QuitServerStream(
...
void QuitServerStream::OnInitialHeadersComplete(
    bool fin, size_t frame_len, const QuicHeaderList& header_list) {
  QUIC_LOG(INFO) << "OnInitialHeadersComplete";
...
void QuitServerStream::OnBodyAvailable() {
...
void QuitServerStream::HandleRequestConnectData(bool fin_received) {
...
void QuitServerStream::SendResponse() {
  QUIC_LOG(INFO) << "SendResponse";
  QUICHE_DCHECK(!IsConnectRequest());
...
QuicConnectionId QuitServerStream::connection_id() const {
  return spdy_session()->connection_id();
}

QuicStreamId QuitServerStream::stream_id() const { return id(); }

std::string QuitServerStream::peer_host() const {
  return spdy_session()->peer_address().host().ToString();
}

QuicSpdyStream* QuitServerStream::GetStream() { return this; }

class DelayedResponseAlarm : public QuicAlarm::DelegateWithContext {
 public:
  DelayedResponseAlarm(QuitServerStream* stream,
 ...
 private:
  QuitServerStream* stream_;
  const QuicBackendResponse* response_;
};

void QuitServerStream::OnResponseBackendComplete(
    const QuicBackendResponse* response) {
  if (response == nullptr) {
...
void QuitServerStream::Respond(const QuicBackendResponse* response) {
...
void QuitServerStream::SendStreamData(absl::string_view data,
                                            bool close_stream) {
...
void QuitServerStream::TerminateStreamWithError(
...
void QuitServerStream::OnCanWrite() {
  QuicSpdyStream::OnCanWrite();
  WriteGeneratedBytes();
}

void QuitServerStream::WriteGeneratedBytes() {
...
void QuitServerStream::SendNotFoundResponse() {
...
void QuitServerStream::SendErrorResponse() { SendErrorResponse(0); }

void QuitServerStream::SendErrorResponse(int resp_code) {
...
void QuitServerStream::SendIncompleteResponse(
...
bool QuitServerStream::IsConnectRequest() const {
  auto method_it = request_headers_.find(":method");
  return method_it != request_headers_.end() && method_it->second == "CONNECT";
}

void QuitServerStream::OnInvalidHeaders() {
  QUIC_DVLOG(1) << "Invalid headers";
  SendErrorResponse(400);
}

const char* const QuitServerStream::kErrorResponseBody = "bad";
const char* const QuitServerStream::kNotFoundResponseBody =
    "file not found";
