...
QuitServer::QuitServer(std::unique_ptr<ProofSource> proof_source,
                       QuicSimpleServerBackend* quic_simple_server_backend)
    : QuitServer(std::move(proof_source),
...
bool QuitServer::CreateUDPSocketAndListen(const QuicSocketAddress& address) {
...
QuicPacketWriter* QuitServer::CreateWriter(int fd) {
  return new QuicDefaultPacketWriter(fd);
}

QuicDispatcher* QuitServer::CreateQuicDispatcher() {
  return new QuitDispatcher(
...
void QuitServer::OnSocketEvent(QuicEventLoop* /*event_loop*/,
                               QuicUdpSocketFd fd,
